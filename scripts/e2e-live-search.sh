#!/usr/bin/env bash
# Exercise debounced live search, session hiding/resume, and session teardown.
...
TMUX_SOCKET="scooter-s2-live-${PPID}-${RANDOM}"
TMUX_SESSION="scooter-s2-live"
...
tmux -L "$TMUX_SOCKET" new-session -d -x 120 -y 40 -s "$TMUX_SESSION" -c "$SEARCH_FIXTURE_DIR" "$HX_BINARY one.txt" \
...
tmux -L "$TMUX_SOCKET" send-keys -t "$PANE_TARGET" 'bet'
